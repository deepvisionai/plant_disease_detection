...
from my_bbox_utils import resize_boxes, normalize_boxes
...
class ODDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        CLASSES = ['Tomato leaf bacterial spot', 'Tomato Septoria leaf spot',
                    'Bell_pepper leaf spot', 'grape leaf', 'Potato leaf early blight',
                    'Strawberry leaf', 'grape leaf black rot', 'Tomato leaf', 'Bell_pepper leaf',
                    'Potato leaf', 'Peach leaf', 'Corn leaf blight', 'Apple rust leaf',
                    'Cherry leaf', 'Tomato Early blight leaf', 'Apple Scab Leaf',
                    'Tomato leaf yellow virus', 'Corn Gray leaf spot', 'Corn rust leaf',
                    'Soyabean leaf', 'Raspberry leaf', 'Blueberry leaf',
                    'Squash Powdery mildew leaf', 'Tomato mold leaf', 'Tomato leaf late blight',
                    'Tomato leaf mosaic virus', 'Potato leaf late blight', 'Apple leaf',
        ...
        if self.transforms:
            for i in range(40):
                # print('ORIG --- ', target['boxes'], '::::', target['dims'])
                ...
                if len(sample['bboxes']) > 0:
                    ...
                    target = resize_boxes(target)
                    # print('RESIZED --- ', target['boxes'], '::::', target['dims'])
                    target = normalize_boxes(target)
                    # print('NORMED --- ', target['boxes'], '::::', target['dims'])
