...
output_files_path = "validation_metric_results"
...
with open(output_files_path + "/output.txt", 'w') as output_file:
    ...
    for class_index, class_name in enumerate(gt_classes):
        ...
        for idx, val in enumerate(tp):
            ...
            # print('GT COUNTER IN LOOP: ', gt_counter_per_class[class_name])
        print('----------------------------------------------')
        print('GT COUNTER: ', gt_counter_per_class[class_name])
        total_gt_objects += gt_counter_per_class[class_name]
        avg_recall = sum(rec) / len(rec)
        print(f"AVERAGE RECALL {class_name}: {avg_recall}")
        prec = tp[:]
        for idx, val in enumerate(tp):
            prec[idx] = float(tp[idx]) / (fp[idx] + tp[idx])
        avg_prec = sum(prec) / len(prec)
        # print(f"AVERAGE PRECISION {class_name}:  {}")
        f1_score = (2*avg_prec*avg_recall) / (avg_prec + avg_recall)
        print(f"F1 SCORE {class_name}: {f1_score}")
        # print('TP', tp)
        # print('LEN TP', len(tp))
...
"""
print('-----------------------------------------------')
total_gt_objects = 2406
print('Total ground truth objects: ', total_gt_objects)
with open(output_files_path + "/output.txt", 'a') as output_file:
    ...
    for class_name in sorted(dr_classes):
        ...
        tn = (total_gt_objects - gt_counter_per_class[class_name]) - (n_det - count_true_positives[class_name])
        # calcuate accuracy of class
        accuracy = (count_true_positives[class_name] + tn) / total_gt_objects
        # print(f"Accuracy of {class_name}: {accuracy}")
        text += ", acc:" + str(accuracy) + ")\n"
        print(text)
        output_file.write(text)
